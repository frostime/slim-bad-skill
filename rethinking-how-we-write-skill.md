---
title: "重新思考SKILL写作：为什么很多旧时代的SKILL其实很“烂”"
source_file: "rethinking-skill_from-workflow-to-contract-blog.docx"
...
**总评：文章的核心判断成立，但需要补强证据边界。**  
它最有价值的命题是：SKILL 不应把一次成功 workflow 固化成未来 Agent 必须执行的路线，而应保存任务契约、边界、验收标准和必要支撑条件。这个框架能解释很多冗长 SKILL 的问题，也能避免“只写目标、不写约束”的反向错误。

### 主要优点

1. **核心区分有效**：文章把“核心契约”和“支撑契约”分开，避免把所有流程都打成坏流程。接口事实、输出 schema、安全门禁、审计字段、真实下游依赖，确实应当进入 SKILL。
2. **例子有解释力**：Ver A / Ver B 的对比抓住了关键差异：前者规定判断原则和决策结果，后者把通用思考过程固定成表演动作。这个例子比抽象讨论更容易让读者理解“路线”和“契约”的区别。
3. **边界处理较完整**：文章没有走向“强 Agent 只需要一句目标”的极端，明确承认测试、工具接口、扫描页渲染、固定 JSON 等约束的必要性。
4. **写作张力强**：标题、例子、反问和结尾判断都有传播性，适合博客语境。

### 主要问题

1. **“能力阈值”论证需要更精确**：文章把前沿模型能力提升概括成工程阈值，这个判断有现实基础，但容易被误读成“所有模型、所有任务都已经不需要 workflow”。建议明确限定为：前沿 Agent、具备工具使用与长程执行能力、任务目标和验收边界清晰的场景。
2. **证据链略偏供应商叙述**：文中引用 Anthropic 和 OpenAI 官方材料是合理的，但它们主要证明“厂商声称能力增强、产品支持长程任务”，不能直接证明“写死 workflow 通常有害”。要支撑后者，最好加入失败案例、A/B 评估、维护成本或迁移成本的经验数据。
3. **语言有几处会降低说服力**：“垃圾”“犯恶心”“血压飙升”等表达能制造情绪强度，但也会让技术读者把注意力转移到态度上。若目标是说服 SKILL 作者，建议把强判断保留在标题或关键句，正文多用可检验标准。
4. **“列表危险”需要降级表达**：问题不在 Markdown 列表本身，而在列表把“回忆性策略”伪装成“规范性约束”。文章后文已经说明这一点，前文可减少对列表形式的泛化攻击。
5. **结尾有一个疑似笔误**：“停止注入哪些手工作坊的经验”更自然应为“停止注入那些手工作坊式的经验”。

### 事实核验

- Anthropic 官方页面显示 Agent Skills 发布于 2025-10-16，并在 2025-12-18 更新中提到将 Agent Skills 发布为跨平台开放标准：[Introducing Agent Skills](https://www.anthropic.com/news/skills)。
- Anthropic 于 2025-11-26 发布《Effective harnesses for long-running agents》，讨论跨多个上下文窗口持续工作的困难，以及 initializer、进度文件、特性列表等 harness 设计：[Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)。
- OpenAI 于 2026-02-23 发布《Run long horizon tasks with Codex》，其中明确使用“steering at milestones instead of micromanaging every line”的表述：[Run long horizon tasks with Codex](https://developers.openai.com/blog/run-long-horizon-tasks-with-codex)。
- OpenAI 于 2026-04-23 发布 GPT-5.5 相关材料，官方介绍和 System Card 都强调其在复杂真实工作、工具使用、检查工作和持续推进方面的增强：[Introducing GPT-5.5](https://openai.com/index/introducing-gpt-5-5/)，[GPT-5.5 System Card](https://openai.com/index/gpt-5-5-system-card/)。

### 修改建议

- 把“模型跨过能力阈值”改成更窄的判断：**在前沿长程 Agent 场景中，过程约束的默认正当性下降。**
- 增加一个反例段：哪些任务仍应强 workflow，例如合规审计、不可逆操作、医疗/法律/财务高风险输出、固定 API 协议、批处理管线。
- 给出一张判定表：某条规则应进入 SKILL 的条件是它来自任务定义、接口事实、因果顺序、安全风险、外部消费者或验收标准；否则默认降级为建议或示例。
- 把情绪性词汇局部收束，以免削弱技术论证的可复用性。

---

# 重新思考SKILL写作：为什么很多旧时代的SKILL其实很“烂”
